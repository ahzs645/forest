...
function issueMatchesContext(issue, state, tags, options = {}) {
  ...
  // Cards that explicitly describe a season must not appear outside it.
  if (issue.seasonLock && issue.seasonBias?.length) {
    const index = Math.max(0, Math.min(SEASONS.length - 1, (state.round || 1) - 1));
    if (!issue.seasonBias.includes(SEASONS[index])) return false;
  }

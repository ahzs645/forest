...
test('escalated field events translate crew-wallet effects to the corporate ledger', () => {
    ...
    options: [
      {
        ...
        chanceSuccess: 0.5,
        chancePartial: 0.3,
        partialEffects: { fuel: -2, budget: -100, crew_health: -2 },
        failureEffects: { fuel: -8, budget: -500, timeUsed: 4 },
        timeUsed: 2,
        riskInjury: 0.2,
  ...
  assert.deepEqual(option.partialEffects, { budget: -1300, crew_morale: -2 });
  assert.deepEqual(option.failureEffects, { budget: -6200 });
  assert.equal(option.chanceSuccess, 0.5);
  assert.equal(option.chancePartial, 0.3);
  assert.equal(option.timeUsed, undefined);
  ...
  assert.deepEqual(event.options[0].partialEffects, { fuel: -2, budget: -100, crew_health: -2 });
});
